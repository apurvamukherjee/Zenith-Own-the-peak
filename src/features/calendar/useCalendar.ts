...
export interface DayCell { date: string; score: number; hasAny: boolean; isToday: boolean; isFuture: boolean }
  ...
  useEffect(() => {
    ...
    computeScoresForMonth(dates, waterGoal, proteinTarget).then((map) => {
      ...
      setCells(dates.map((date) => ({
        date,
        score: map.get(date)?.score ?? 0,
        hasAny: map.get(date)?.hasAny ?? false,
        isToday: date === today,
        isFuture: date > today,
      })));
    });
