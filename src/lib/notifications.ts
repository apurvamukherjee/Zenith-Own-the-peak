...
export function planReminders(c: ReminderConfig, includeSupps = true): PlannedReminder[] {
  const out: PlannedReminder[] = [];
  let id = 1000;
  if (c.water) {
    const end = c.wakeHour + c.wakingWindowH;
    for (let h = c.wakeHour + c.waterEveryH; h < end; h += c.waterEveryH) {
      out.push({ id: id++, title: "💧 Hydrate", body: "Time for a few sips — stay on pace.", at: hm(h % 24) });
    }
  }
